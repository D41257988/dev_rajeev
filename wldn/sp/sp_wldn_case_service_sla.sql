...
declare load_source string default 'trans_crm_mart.sp_wldn_case_service_sla';
...
declare job_type string default 'CASE-SERVICE-SLA';
...
delete from `rpt_crm_mart.t_wldn_case_sla` where week_begin = cast(replace(cast(v_startdate as string), '-', '') as int64) and case_type_category = 'SC';
...
case_sla_nor_flag as (
  ...
  select
    * except(start_status, start_date),
    -- case when (lag(sla_start) over (partition by case_id order by row_num)=1 and sla_start =1)
    -- then LAG(start_status) over (partition by case_id order by row_num) else start_status end as start_status_old,
    -- case when (lag(sla_start) over (partition by case_id order by row_num)=1 and sla_start =1)
    -- then LAG(start_date) over (partition by case_id order by row_num) else start_date end as start_date_old,
    ...
    -- when (start_status in ('New', 'Re-Open', 'Open', 'Work Completed') and end_status in ('New', 'Re-Open', 'Open',  'Work Completed') )
    -- then LAG(start_status) over (partition by case_id order by row_num)
...
-- coi.type_action_c, rt.name as record_type, u.manager_id
  ...
  where
    ...
    and (coi.institution_brand_c in ('a0kDP000008l7bvYAA') OR coi.institution_code_c = 3.0)
...
exception when error then
  ...
  insert into `audit_cdw_log.audit_load_details` (audit_load_key, job_name, job_start_dt, job_end_dt, job_completed_ind, job_type, load_method, load_source)
  values (upper(replace(generate_uuid(), "-", "")), 't_wldn_case_sla', current_timestamp(), cast(NULL as TIMESTAMP), 'N', 'CASE-SERVICE-SLA', 'scheduled query', 'trans_crm_mart.sp_wldn_case_service_sla');
  ...
  values
  (upper(replace(generate_uuid(), "-", "")),'CASE-SERVICE-SLA', 't_wldn_case_sla', @@error.statement_text, @@error.message, current_timestamp() ,'trans_crm_mart.sp_wldn_case_service_sla') ;
