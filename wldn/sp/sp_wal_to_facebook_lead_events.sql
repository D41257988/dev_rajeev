CREATE OR REPLACE PROCEDURE `trans_bi_offline_conversion.sp_wal_to_facebook_lead_events`(IN AUDITKEY STRING, OUT RESULT STRING)
OPTIONS (strict_mode=false)
begin

declare v_startdate timestamp default current_timestamp();
declare fromdate date default date_add(current_date("America/New_York"), interval -2 day); --'2021-10-16'


insert into `trans_bi_offline_conversion.wal_to_facebook_lead_events`
(
    ...
    crm_stage_disposition,is_walden_alum, client_user_agent,action_source,event_source_url,etl_created_date,etl_ins_audit_key,
    etl_resource_name
)
        ...
        -- pull opps created on fromdate
            select opp.id, opp.created_date, opp.stage_name, opp.disposition_c, opp.contact_c, opp.system_modstamp, opp.Program_of_Interest_c, opp.channel_c,
            opp.client_user_agent_c, opp.event_source_url_c , null as field, opp.transaction_id_c, opp.app_start_event_id_c, opp.app_submit_event_id_c
...
/*
            -- pull Alumni opps (contact.is_walden_alum_c)
            select opp.id, opp.created_date, opp.stage_name, opp.disposition_c, opp.contact_c, opp.system_modstamp, opp.Program_of_Interest_c, opp.channel_c,
            opp.client_user_agent_c, opp.event_source_url_c
            from `raw_b2c_sfdc.opportunity` opp
            join `raw_b2c_sfdc.contact` c
            on c.id = opp.contact_c
            where left(string(c.system_modstamp,"America/New_York"), 10) = cast(fromdate as string)
            and is_walden_alum_c
...
*/
            ...
            select opp.id, opp.created_date, opp.stage_name, opp.disposition_c, opp.contact_c, opp.system_modstamp, opp.Program_of_Interest_c, opp.channel_c,
            opp.client_user_agent_c, opp.event_source_url_c ,ofh.field, opp.transaction_id_c, opp.app_start_event_id_c, opp.app_submit_event_id_c
            from `raw_b2c_sfdc.opportunity` opp
            join `raw_b2c_sfdc.opportunity_field_history` ofh on opp.id = ofh.opportunity_id
         ...
         -- pull opps that had a Application Started on fromdate
            select  opp.id, opp.created_date, 'Applicant' as stage_name, ofh_disp.old_value as disposition_c, opp.contact_c, opp.system_modstamp, opp.Program_of_Interest_c, opp.channel_c,
            opp.client_user_agent_c, opp.event_source_url_c ,ofh.field, opp.transaction_id_c, opp.app_start_event_id_c, opp.app_submit_event_id_c
            from `raw_b2c_sfdc.opportunity` opp
            ...
            join (select  opportunity_id, created_date,
                        field,  old_value , row_number() over(partition by opportunity_id order by created_date ) rn
            ...
            and rn = 1
            where left(string(ofh.created_date,"America/New_York"), 10) = cast(fromdate as string) and opp.is_deleted=false and opp.institution_c='a0ko0000002BSH4AAO'


        ),
         opp_stage as (
            select opps.id, ofh.created_date,
            ofh.field, new_value
            from opps join
        ...
        stage_disposition_combined as (
         select opportunity_id, created_date, max(ofh_created_date) as ofh_created_date,  disposition, stage, contact_c, Program_of_Interest_c, channel_c, client_user_agent_c, event_source_url_c ,transaction_id_c, app_start_event_id_c, app_submit_event_id_c
           from(
            select
            ofh.opportunity_id,
            opps.created_date,
            ...
            and left(string(ofh.created_date,"America/New_York"), 10) = cast(fromdate as string)
     --       and ifnull(opp_stage.created_date,opps.created_date) <= ifnull(ofh.created_date,opps.created_date)
            )
            group by opportunity_id, created_date,  disposition, stage, contact_c, Program_of_Interest_c, channel_c, client_user_agent_c, event_source_url_c , field, transaction_id_c, app_start_event_id_c, app_submit_event_id_c
        ),  Begin_opp_stage as (
            select opps.id, ofh.created_date,
            ofh.field,  old_value , row_number() over(partition by ofh.opportunity_id order by ofh.created_date ) rn
            from opps join
            `raw_b2c_sfdc.opportunity_field_history` ofh
            on opps.id = ofh.opportunity_id
        ...
        all_stage_disposition as (
            select opportunity_id, created_date, ofh_created_date,  disposition,case when disposition = 'Complete - EA Ready for Review' then 'Applicant' else stage end as stage
                , contact_c, Program_of_Interest_c, channel_c, client_user_agent_c, event_source_url_c, transaction_id_c, app_start_event_id_c, app_submit_event_id_c
            from stage_disposition_combined
            where ((disposition = 'Complete - EA Ready for Review') or
                    (stage = 'Pre-Enroll' and disposition = 'Reserved'))
        ...
        from (select opportunity_id, created_date, ofh_created_date,  disposition, stage, contact_c, Program_of_Interest_c, channel_c, client_user_agent_c, event_source_url_c , transaction_id_c, app_start_event_id_c, app_submit_event_id_c,
	...
	-- pick begin stage and disposition
        ...
        contact_phone_list as (
            ...
            [coalesce(safe_cast(regexp_replace(preferred_phone_number_c, '[^[:digit:]]', '') as int64),null),
                ...
                coalesce(safe_cast(regexp_replace(c.work_phone_c, '[^[:digit:]]', '' ) as int64),null)]
            as phone_numbers_list
        ...
        contact_phone_list_final as (
            select id, array_agg(distinct string ignore nulls) as phone_numbers
        ...
        contact_email_list_final as (
            select id, array_agg(distinct string ignore nulls) as emails
     ...
     as (
        ...
        ToF.score as lead_score_tof,
        dls.score as lead_score_dynamic,
        ...
        case when all_stage_disposition.client_user_agent_c is not null then 'Website' else 'Other' end as action_source,
    ...
    ------------for manually inserting (Testing) ------------------------            '22f51867-f40b-4d95-b4f2-3fdb3ac9930c' as etl_ins_audit_key ,  --
        ...
        all_stage_disposition join
        `raw_b2c_sfdc.contact` c on all_stage_disposition.contact_c = c.id and c.is_deleted = false

        join `raw_b2c_sfdc.product_2` p on all_stage_disposition.Program_of_Interest_c = p.id and c.is_deleted = false

        left join lead_score_tof ToF on all_stage_disposition.opportunity_id = ToF.sf_id

        left join lead_score_dlf dls on all_stage_disposition.opportunity_id = dls.sf_id

        left join contact_phone_list_final  on all_stage_disposition.opportunity_id = contact_phone_list_final.id

        left join contact_email_list_final on all_stage_disposition.opportunity_id = contact_email_list_final.id
	...
	-- only events created currentdate - 2 and have below events in addition to lead at the same time
        ...
        -- event_id should be unique. It is used for matching on facebook side
        COALESCE(all_stage_disposition.transaction_id_c, concat(all_stage_disposition.opportunity_id,'L')) as event_id,
    ...
    ------------------------------------Test Audit -----------------------------            '22f51867-f40b-4d95-b4f2-3fdb3ac9930c' as etl_ins_audit_key ,  --
        ...
        Begin_stage_disposition_combined  all_stage_disposition join
        `raw_b2c_sfdc.contact` c
        on all_stage_disposition.contact_c = c.id and c.is_deleted = false

        join `raw_b2c_sfdc.product_2` p
        on all_stage_disposition.Program_of_Interest_c = p.id and c.is_deleted = false
       ...
       select f.* except (rn)
        from (select * from
                (select row_number()  over (partition by left(event_id,18), event_name order by event_time) as rn ,
                    * from Final
   ...
   --    where left(event_id,18) = '0062G00000mBz69QAC' --'0062G00000mBz69QAC%'
       ...
       /************ left join to ensure only records which were not sent successfully previously are sent this time**********/
        left join `trans_bi_offline_conversion.wal_to_facebook_lead_events_api_response` wfl
		on left(wfl.event_id,18) = left(f.event_id,18) -- send one oppid-eventname only once.
        ...
        where wfl.event_id is null -- only record/send the first occurance of the event for an opportunity
            ;
