...
begin
    ...
    declare target_tablename string default 't_wldn_daily_nh_funnel_data';
    declare source_tablename string default 'temp_walden_daily_nh_funnel_data';
    declare load_source string default 'trans_crm_mart.sp_wldn_daily_nh_funnel_data';
...
select
  'Previous' as Period
	,DATE_ADD( cur_begin,INTERVAL -1 YEAR  ) as Begin_dttm
	,DATE_ADD( cur_end,INTERVAL -1 YEAR  ) as End_dttm
	,DATE_ADD( cur_status_cutoff,INTERVAL -1 YEAR  ) as Status_cutoff_dttm
	,DATE_ADD( cur_enr_cutoff,INTERVAL -1 YEAR  ) as Enr_cutoff_dttm
)
select *
...
left join raw_b2c_sfdc.brand_profile_c t3 on t1.brand_profile_sfid=t3.id and t3.is_deleted=false
...
leads_reinq3 as (
 ...
 leads_reinq4 as (
  ...
  where lower(activity_id_c) not in ('unknown','-99999999') and is_deleted=false
	and institution_c = 'a0ko0000002BSH4AAO'
  ) t2
...
*/
CREATE
OR REPLACE temp TABLE `temp_walden_daily_nh_funnel_data`
AS
with
leads_reinq7 as (
	select t1.*
