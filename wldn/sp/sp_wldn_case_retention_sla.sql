CREATE OR REPLACE PROCEDURE `trans_crm_mart.sp_wldn_case_retention_sla`()
begin
...
declare load_source string default 'trans_crm_mart.sp_wldn_case_retention_sla';
...
declare job_type string default 'CASE-RETENTION-SLA';
...
delete from `rpt_crm_mart.t_wldn_case_sla` where week_begin = cast(replace(cast(v_startdate as string), '-', '') as int64) and case_type_category = 'RC';
...
with unique_case as (
  ...
  from
    `raw_b2c_sfdc.case` C
    -- `stg_l1_salesforce.v_case_service` C
...
cte_last_obj as (
  ...
  select
    case_id, max(created_by_id) over(partition by case_id order by created_date desc ) as created_by_id,
...
cte_last_user as (
  ...
  select
    ...
    ifnull(u.department_c, u.department) as last_modified_by_dept,
    m.name as last_modified_by_manager
  from
    cte_last_obj cte
  left join
...
case_sla_nor_flag as (
  ...
  select
    ...
    -- case when start_status in ('New', 'Re-Open', 'Open', 'Transferred') then 1 else 0 end as sla_start,--nor_flag,
    case when start_status in ('Re-Open') then 1 else 0 end as sla_start,--nor_flag,
...
case_sla_rpt_dates as (
  ...
  select
    ...
    -- case  when (is_dept_weekend_working = 'Y' and ned.holiday_date is not null ) then cast(concat(ned.nxt_day_holiday, ' 00:00:00 UTC') as timestamp)
    --       when (is_dept_weekend_working != 'Y' and ned.holiday_date is not null ) then cast(concat(ned.nxt_day_weekend_holiday, ' 00:00:00 UTC') as timestamp)
    --       when extract(dayofweek from end_date) = 1 then cast(concat(date_add(date(end_date), interval 1 day), ' 00:00:00 UTC') as timestamp)
    --       when extract(dayofweek from end_date) = 7 then cast(concat(date_add(date(end_date), interval 2 day), ' 00:00:00 UTC') as timestamp)
    --       else end_date end rpt_sla_end_date,

    csla.end_date as rpt_sla_end_date,

    -- nsd.holiday_date as start_holiday,
    -- nsd.day_add_weekend_holiday,
    -- nsd.day_add_holiday,
    -- ned.holiday_date as end_holiday,
    -- ned.day_add_weekend_holiday, ned.day_add_holiday
...
case_sla_rpt_planned_end_temp as (
  ...
  select
    *,
    -- case when date_diff(date(temp_planned_sla_end_date), date(rpt_sla_start_date), week) > 0
    -- then
    --   case
    --     when
    --       extract(dayofweek from temp_planned_sla_end_date) in (1,7)
    --     then
    --       (((date_diff(date(temp_planned_sla_end_date), date(rpt_sla_start_date), week) * 2) -1)) * 24
    --     else
    --       ((date_diff(date(temp_planned_sla_end_date), date(rpt_sla_start_date), week) * 2) ) * 24 end
    -- else
    --   0
    -- end as temp_planned_weekends_old,
...
temp_case_status_sla as (
  ...
  select
    ...
    -- 'trans_crm_mart.sp_wldn_case_service_sla' as etl_resource_name,
    -- v_audit_id as etl_ins_audit_key,
    -- cast(null as string) etl_upd_audit_key

  from
    case_sla_rpt_planned_end_wknd_holiday
  -- where (end_status not in ('New', 'Re-Open', 'Open', 'Transferred', 'Work Completed') or end_status is null)
  where (end_status not in ('New', 'Re-Open', 'Open', 'Transferred') or end_status is null)
...
case_tbl_closed_cases as (

  select
    coi.id as case_id,
    coi.type as type,
    coi.subtype_c as subtype,
    coi.type_action_c as type_action,
    coi.banner_id_c as banner_id,
    coi.gsfs_action_c as gsfs_action,
    coi.gsfs_action_outcome_c as gsfs_action_outcome,
    rt.name as record_type,
    coi.original_record_type_c as original_record_type,
    coi.case_number as case_number,
    coi.origin as origin,
    coi.case_closed_reason_c as case_closed_reason,
    coi.status as case_status,
    coi.parent_id as parent_id,
    'Closed' as start_status,
    'Closed' as end_status,
    coi.owner_id as owner_id,
    ifnull(concat(u.first_name,' ', u.last_name), gp.name) as owner_name,
    m.name as owner_manager,
    ifnull(ifnull(u.department, u.department_c),loe_q.departmentname) as department,
    cast(`utility.udf_convert_UTC_to_EST`(coi.closed_date) as timestamp) as actual_sla_start,
    cast(`utility.udf_convert_UTC_to_EST`(coi.closed_date) as timestamp) as actual_sla_end,
    0.0 as actual_response_duration,
    cast(null as INT64) as slahrs,
    24 as default_sla_hrs,
    cast(`utility.udf_convert_UTC_to_EST`(coi.closed_date) as timestamp) as rpt_sla_start_date,
    cast(`utility.udf_convert_UTC_to_EST`(coi.closed_date) as timestamp) as rpt_sla_end_date,
    0.0 as rpt_response_duration,
    cast(null as timestamp) as planned_sla_end_date,
    'Y' as in_sla

-- coi.type_action_c, rt.name as record_type, u.manager_id
  -- coi.id, coi.case_number, coi.type, coi.subtype_c, coi.type_action_c, origin, parent_id, record_type_id, rt.name as record_type, status, reason, cast(`utility.udf_convert_UTC_to_EST`(coi.created_date) as timestamp) as created_date_est, coi.owner_id,
  -- ifnull(concat(u.first_name,' ', u.last_name), gp.name) as owner_name, ifnull(ifnull(u.department, u.department_c),loe_q.departmentname) as department , u.manager_id
  from
    `raw_b2c_sfdc.case` coi
  left join
    `raw_b2c_sfdc.user` u on coi.owner_id = u.id
  left join
    `raw_b2c_sfdc.group` gp on coi.owner_id = gp.id
  left join
    -- `trans_edw_sm_dm_dbo.dim_loe_queue` loe_q on lower(gp.name) = lower(loe_q.name)
    `rpt_crm_mart.v_wldn_queue` loe_q on lower(gp.name) = lower(loe_q.name)
  left join
  ...
  where
    --  cast(`utility.udf_convert_UTC_to_EST`(coi.created_date) as date) >= v_startdate and cast(`utility.udf_convert_UTC_to_EST`(coi.created_date) as date) < v_enddate
    --  cast(`utility.udf_convert_UTC_to_EST`(coi.last_modified_date) as date) >= v_startdate and cast(`utility.udf_convert_UTC_to_EST`(coi.last_modified_date) as date) < v_enddate
      cast(`utility.udf_convert_UTC_to_EST`(coi.closed_date) as date) >= v_startdate and cast(`utility.udf_convert_UTC_to_EST`(coi.closed_date) as date) < v_enddate
    -- and coi.id not in (select distinct case_id from temp_case_status_sla )
    -- and coi.id not in (select distinct case_id from case_resolution_history )
    and coi.id not in (select distinct case_id from case_history where new_value is not null and old_value is not null )
...
select
  5 as institution_id,
  'WLDN' as institution,
  cast(replace(cast(v_startdate as string), '-', '') as int64) as week_begin,
  t1.case_id,
  type,
  subtype,
  type_action,
  ...
  'retention' as case_type,
  'RC' as case_type_category,
  current_timestamp() as etl_created_date,
  cast(null as timestamp) as etl_updated_date,
  'trans_crm_mart.sp_wldn_case_retention_sla' as etl_resource_name,
  v_audit_id as etl_ins_audit_key,
  cast(null as string) etl_upd_audit_key
...
from
  case_tbl_closed_cases t2
left join
  cte_last_user clu on t2.case_id = clu.case_id
left join
  cte_case_closed_reason ccr on t2.case_id = ccr.case_id
left join
  cte_sla_out_owner csoo on t2.case_id = csoo.case_id
where
  start_status = 'Re-Open'
order by
  actual_sla_start;
...
exception when error then
  ...
  insert into `audit_cdw_log.audit_load_details` (audit_load_key, job_name, job_start_dt, job_end_dt, job_completed_ind, job_type, load_method, load_source)
  values (upper(replace(generate_uuid(), "-", "")), 't_wldn_case_sla', current_timestamp(), cast(NULL as TIMESTAMP), 'N', 'CASE-RETENTION-SLA', 'scheduled query', 'trans_crm_mart.sp_wldn_case_retention_sla');
  ...
  values
  (upper(replace(generate_uuid(), "-", "")),'CASE-RETENTION-SLA', 't_wldn_case_sla', @@error.statement_text, @@error.message, current_timestamp() ,'trans_crm_mart.sp_wldn_case_retention_sla') ;
