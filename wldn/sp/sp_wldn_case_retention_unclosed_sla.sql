CREATE OR REPLACE PROCEDURE `daas-cdw-dev.trans_crm_mart.sp_wldn_case_retention_unclosed_sla`()
begin
...
declare load_source string default 'trans_crm_mart.sp_wldn_case_retention_unclosed_sla';
...
declare job_type string default 'CASE-RETENTION-UNCLOSED-SLA';
...
/*
set v_startdate = '2016-01-01';
set v_weekbegindate = '2023-03-25';
set v_enddate = '2023-03-25'; */
...
delete from `rpt_crm_mart.t_wldn_case_sla` where week_begin = cast(replace(cast(v_weekbegindate as string), '-', '') as int64) and case_type_category = 'RC-UC';
...
case_history_raw as (
  ...
  from
    unique_case uc
  left join
    `raw_b2c_sfdc.case_history` ch on uc.id = ch.case_id
  where
...
cte_last_user_temp as (
  ...
  select
    *,
    max(row_num) over(partition by case_id) as row_max_last,
  from
...
case_history_iscreated as (
  ...
  from
    case_history cht
  inner join case_iscreated cic on cht.case_id = cic.case_id
...
case_dep as (
  ...
  select
    ...
    concat(u.first_name,' ', u.last_name) as owner_name, u.department, u.department_c , gp.name, coi.row_num, coi.created_date, loe_q.departmentname, m.name as owner_manager
  from
    case_owner_info coi
  left join
...
select
  ...
  'RC-UC' as case_type_category,
  current_timestamp() as etl_created_date,
  cast(null as timestamp) as etl_updated_date,
  'trans_crm_mart.sp_wldn_case_retention_unclosed_sla' as etl_resource_name,
  v_audit_id as etl_ins_audit_key,
  cast(null as string) etl_upd_audit_key
...
where
  start_status = 'Re-Open' and end_status is null;
...
exception when error then
  ...
  insert into `audit_cdw_log.audit_load_details` (audit_load_key, job_name, job_start_dt, job_end_dt, job_completed_ind, job_type, load_method, load_source)
  values (upper(replace(generate_uuid(), "-", "")), 't_wldn_case_sla', current_timestamp(), cast(NULL as TIMESTAMP), 'N', 'CASE-RETENTION-UNCLOSED-SLA', 'scheduled query', 'trans_crm_mart.sp_wldn_case_retention_unclosed_sla');
  ...
  values
  (upper(replace(generate_uuid(), "-", "")),'CASE-RETENTION-UNCLOSED-SLA', 't_wldn_case_sla', @@error.statement_text, @@error.message, current_timestamp() ,'trans_crm_mart.sp_wldn_case_retention_unclosed_sla') ;
