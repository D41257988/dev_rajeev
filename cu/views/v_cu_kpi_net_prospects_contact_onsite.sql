...
FROM
  ...
  SELECT
    ...
    d.writing_period
  FROM
    UNNEST(rpt_crm_mart.f_cu_crm_net_prospects_task('CONTACTED',
        'DIALER CONTACTED',
        'ASC CONTACTED')) v
  LEFT JOIN (
    SELECT
  ...
  ON
    ( EXTRACT(date
      ...
      AND EXTRACT(date
      FROM
        CAST(DATETIME(task_created_date, "US/Central") AS datetime)) BETWEEN wp_begin_date
      AND DATE_ADD(wp_begin_date, INTERVAL 140 day) )
  LEFT JOIN (
    SELECT
